...
Shows what changed in `.env.example` since the last git commit — added, removed, or modified variables.

```bash
env-doctor diff [options]
...
| Code | Meaning |
|------|---------|
| `0`  | No changes since last commit |
| `1`  | Changes detected |

### `env-doctor audit`

Scans your `.env` file values for real leaked credentials — API keys, tokens, passwords, and private keys. Catches secrets accidentally committed or left in local config.

```bash
env-doctor audit [options]
...
### `env-doctor generate-validator`

Generates a small runtime script that checks for required environment variables when your app starts. If any are missing, it prints a clear error and exits immediately — before anything else runs.

This is the bridge between static analysis (what env-doctor checks at dev time) and production: the generated script runs on **any server, any deployment, any CI environment** automatically at startup.

```bash
env-doctor generate-validator [options]

Options:
  --example-file <path>    Path to .env.example to read keys from (default: .env.example)
  --output <path>          Output file path (default: env-validator.js or env-validator.ts)
  --typescript             Generate TypeScript instead of JavaScript
  --no-color               Disable ANSI color output
  --root <path>            Project root directory (default: cwd)
```

**Example — generate the validator:**

```bash
npx @neerav34/env-doctor generate-validator
# → writes env-validator.js to your project root
```

**Generated file (`env-validator.js`):**
...
```js
// index.js or server.js — first line
...
```ts
// index.ts or server.ts — first line
...
```bash
npx @neerav34/env-doctor generate-validator --typescript
# → writes env-validator.ts
```

**Re-generate after updating `.env.example`:**
